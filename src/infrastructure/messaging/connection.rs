...
pub async fn connect(url: &str) -> Result<Client> {
    ...
    options.connect(url).await.map_err(|e| {
        ...
        MessagingError::ConnectionFailed { url: url.to_string(), source: e }
    })
}

/// Publish a message to NATS (Fire and Forget)
pub async fn publish(client: &Client, subject: String, payload: bytes::Bytes) -> Result<()> {
    client.publish(subject.clone(), payload).await.map_err(|e| {
        error!("Failed to publish to {}: {}", subject, e);
        MessagingError::PublishFailed { subject, source: e }
    })?;
    Ok(())
}
...
pub async fn subscribe(client: &Client, subject: String) -> Result<async_nats::Subscriber> {
    client.subscribe(subject.clone()).await.map_err(|e| {
        error!("Failed to subscribe to {}: {}", subject, e);
        MessagingError::SubscribeFailed { subject, source: e }
    })
}
