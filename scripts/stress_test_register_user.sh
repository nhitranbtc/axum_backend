...
# gRPC User Registration Stress Test (Optimized)
# Tests the gRPC UserService/CreateUser endpoint under load
# This is an alias for stress_test_create_user.sh with a different name
...
# Use timestamp as unique test ID
TEST_ID="$TIMESTAMP"

echo -e "${BLUE}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║     gRPC User Registration Stress Test (Optimized)    ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════╝${NC}"
echo ""
...
create_user() {
    ...
    local response=$(grpcurl -plaintext \
        -d "{\"email\": \"stress_test_${TEST_ID}_${user_id}@example.com\", \"name\": \"Stress Test User ${user_id}\", \"password\": \"SecurePassword123!\"}" \
        "$GRPC_HOST" \
...
export GRPC_HOST TEMP_DIR TEST_ID
...
TOTAL_REQUESTS=$(tail -n +2 "$TEMP_DIR/results.csv" 2>/dev/null | wc -l)
...
# Avoid division by zero for throughput
