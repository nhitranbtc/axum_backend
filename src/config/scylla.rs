...
impl ScyllaConfig {
    ...
    pub fn from_env() -> anyhow::Result<Self> {
        let nodes_str =
            env::var("SCYLLA_NODES").map_err(|_| anyhow::anyhow!("SCYLLA_NODES must be set"))?;

        let nodes = nodes_str.split(',').map(|s| s.trim().to_string()).collect();
        ...
        Ok(Self { nodes, keyspace, username, password, replication_factor })
    }
}
